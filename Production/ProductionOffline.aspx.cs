...
{
    public partial class ProductionOffline : Ebiz.Scaffolding.WebForm.UI.CustomPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    _isMechLine = ProductionRepository.IsMechLineStation(_stationId);
        ...
        private void loadDetail()
        {
            ...
            if (stations != null)
            {
                ...
                lblStation.Text = "[Offline]" + stations.Current.StationName + (_isSubAssy ? string.Empty : " [" + stations.Current.LineId + "]");
                ...
                if (_isMechLine == true)
                {
                    ...
                    btnSendMech.Visible = true;
                   // btnSendToMechHeader.Visible = true;
                }
                else
                {
                    SubmitButton.Visible = true;
                    btnSendMech.Visible = false;
                  //  btnSendToMechHeader.Visible = false;
                    //btnSendToTrim.Visible = true;
                }
            }
        }
        ...
        protected void BtnEndOffLine_Click(object sender, EventArgs e)
        {
            ...
            var userauthkey = ProductionRepository.CheckAuthOrg(tbTrim.Text);
            ...
            if (_isMechLine == true)
            {
                PopUpSendToMech.ShowOnPageLoad = true;
                PopUpEndOffLine.ShowOnPageLoad = false;
            }
            else
            {
                PopUpEndOffLine.ShowOnPageLoad = true;
                PopUpSendToMech.ShowOnPageLoad = false;
                TrimPopUp.ShowOnPageLoad = false;
                SubmitButton.Visible = true;
            }
            TrimPopUp.ShowOnPageLoad = false;
            MechValidasi.ShowOnPageLoad = false;
                //PopUpEndOffLine.ShowOnPageLoad = false;
        ...
        protected void btnSendToTrim_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (!status)
                {
                    ScriptManager.RegisterClientScriptBlock(this, GetType(), "alertMessage", @"alert('Failed Move to Station, please contact your administrator');", true);
                    return;
                }
            }
            catch (Exception ex)
            {
                ScriptManager.RegisterClientScriptBlock(this, GetType(), "alertMessage", @"alert('Failed Move to Station: " + ex.Message + "');", true);
                AppLogger.LogError(ex.Message);
                return;
            }

            ScriptManager.RegisterClientScriptBlock(this, GetType(), "alertMessage", @"alert('Successfully move !');window.location.assign('../Production/ProductionDashboard.aspx');", true);
            //Response.Redirect("../Production/ProductionDashboard.aspx");
        }

        protected void btnSendMech_Click(object sender, EventArgs e)
        {
            try
            {
                var _StationPick = cmb_MechStation.SelectedItem.Value;
                var _stationId = Convert.ToInt32(Request.QueryString["Stat"]);
                var _historyId = Convert.ToInt32(Request.QueryString["Hist"]);
                //not end of line

                if (ProductionRepository.CheckValidasi(Convert.ToInt32(_StationPick), _lineId))
                {
                    // var _StationPick = IdcmbEndOffLine.SelectedItem.Value;
                    var _StationPickName = cmb_MechStation.SelectedItem.Text;
                    var _Sequence = ProductionRepository.GetStationCode(_StationPick.ToString());
                    //convert object to readable type
                    ...
                    //insert new station detail to start eol
                    if (ProductionRepository.UpdateProductionReworksOut(_historyId, "OFFLINE", DateTime.Now, DateTime.Now))
                    //back to dashboard
                    {
                        if (ProductionRepository.__InsertProductionHistoryDetailSendToTrim(histItem.ItemId, newStationId,
                             false, false, ""))
                        {
                            //update last un done history detail
                            if (ProductionRepository.UpdateProductionHistoryDetailFromFault(histItem.ItemId, histItem.StationId,
                                histItem.IsOffline, histItem.IsRectification, histItem.IsOffline, true, true))
                            {
                                ProductionRepository.UpdateAllSubAssyBySerialNumber(histItem.FINNumber);
                                ProductionRepository.UpdateStationPick(_historyId.ToString(), _StationPick.ToString(), _StationPickName.ToString(), _Sequence.ToString());
                    ...
                    {
                        ScriptManager.RegisterClientScriptBlock(this, GetType(), "alertMessage4", @"alert('Any Process In Next Station');", true);
                ...
                {
                    ScriptManager.RegisterClientScriptBlock(this, GetType(), "alertMessage4", @"alert('Cannot Update in Reworksata');", true);
                    pcLogin.ShowOnPageLoad = false;
                    return;
                }
        ...
        protected void PopUpSendToMech_WindowCallback(object source, PopupWindowCallbackArgs e)
        {

        }

        protected void BtnValidasiMech_Click(object sender, EventArgs e)
        {
            ASPxButton btn = (sender as ASPxButton);

            var conditionalBtn = btn.CommandArgument;

            var userauthkey = ProductionRepository.CheckAuthOrg(tbMechText.Text);
        ...
        protected void BtnSendToStation(object sender, EventArgs e)
        {
            var user = ProductionRepository.GetUserQM(TxAuthKeyBackStation.Text.Trim());
            if (user == null)
            {
                ScriptManager.RegisterClientScriptBlock(this, GetType(), "alertMessage2", @"alert('Your Auth Key not valid');", true);
                tbLogin2.Text = "";
            ...
            try
            {
                bool status = prodHist.OfflineToStation(userName);
                if (!status)
                {
                    ScriptManager.RegisterClientScriptBlock(this, GetType(), "alertMessage", @"alert('Failed Move to Station, please contact your administrator');", true);
                    return;
        ...
        protected void ToStation_Click(object sender, EventArgs e)
        {
            var user = ProductionRepository.GetUserQM(tbLogin2.Text.Trim());
            if (user == null)
            {
